...
> **说明**：**`memory`** 子系统含 **`classify` / `memgraph` / `oom` / `javamem`**（默认**快速排查**）；可选 **`--deep-diagnosis`** 接**深度诊断**（远程专项）。非内存 Agent 主路径为 **`io` / `net` / `load`**。底层 OpenAPI 直调 CLI 不向 Agent 宣读，维护者见 [invoke-diagnosis.md](../../references/invoke-diagnosis.md)。见 [SKILL.md](../../SKILL.md)。权限引导见 [openapi-permission-guide.md](../references/openapi-permission-guide.md)。
...
2. **子系统命令**（Subsystem Command）
   ...
   - 使用方式：非内存 `osops io|net|load <service_name> ...`；维护者 OpenAPI 直调见 [invoke-diagnosis.md](../../references/invoke-diagnosis.md)
...
```bash
export PYTHONPATH="/path/to/sysom-diagnosis/shared/scripts:$PYTHONPATH"
```
...
```bash
# 查看帮助
./shared/scripts/osops memory leak --help

# 执行命令
./shared/scripts/osops memory leak --pid 1234 --duration 120
```
...
```bash
# 查看帮助
./shared/scripts/osops version --help

# 执行命令
./shared/scripts/osops version
```
...
```bash
# 默认就是 local 模式
./shared/scripts/osops memory oom

# 或显式指定
MEMORY_MODE=local ./shared/scripts/osops memory oom
```
...
```bash
MEMORY_MODE=remote ./shared/scripts/osops memory cache
```
...
```bash
MEMORY_MODE=hybrid ./shared/scripts/osops memory oom
```
...
# 测试命令执行
MEMORY_MODE=local ./shared/scripts/osops memory leak --pid 1234
...
```bash
# 输出 JSON 并格式化
./shared/scripts/osops memory classify | python3 -m json.tool
```
