...
def recommended_specialty_cli_command(
    ...
    memory_cli = {
        "memgraph": f"./shared/scripts/osops memory memgraph --deep-diagnosis {suffix}",
        "oomcheck": f"./shared/scripts/osops memory oom --deep-diagnosis {suffix}",
        "javamem": f"./shared/scripts/osops memory javamem --deep-diagnosis {suffix}",
    }
    cmd = memory_cli.get(
        service_name,
        f"./shared/scripts/osops memory classify --deep-diagnosis {suffix}",
    )
    if ns is not None:
...
def build_next_steps_commands(
    ...
    return [
        {
            "action": "precheck",
            "command": "./shared/scripts/osops precheck",
            "purpose_zh": pre,
        },
        {
            "action": "diagnosis_invoke",
