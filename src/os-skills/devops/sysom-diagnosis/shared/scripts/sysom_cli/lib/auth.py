"""
认证检查器
支持检查 AKSK 和 ECS RAM Role 两种认证方式，并调用 SysOM API 验证权限
"""
import os
import json
...
def check_env_credentials() -> Dict[str, Any]:
    ...
    return {"available": False, "method": "环境变量"}
...
def check_aliyun_config() -> Dict[str, Any]:
    ...
    2. EcsRamRole 模式：mode=EcsRamRole，SDK 自动从元数据服务获取临时凭证
    3. 兼容旧格式：包含 ram_role_name 字段（不推荐）
    ...
    try:
        ...
        for profile in config.get("profiles", []):
            if profile.get("name") == profile_name:
                ...
                # AK 模式
                if mode == "AK" and "access_key_id" in profile:
                    return {
                        "available": True,
                        "method": "配置文件(AK)",
                        "credentials": {
...
def generate_help_message(
    *,
    ...
    aksk = (
        ...
        "A. 推荐（跨 Shell / Agent）：./shared/scripts/osops configure，写入 ~/.aliyun/config.json\n"
        ...
        "C. 或手工编辑 ~/.aliyun/config.json（mode=AK）"
    )
    ram_role = (
    ...
    ram_role_compact = (
        "方式 2: ECS RAM Role — 完整步骤见 shared/references/authentication.md（ECS RAM Role）。"
    )
    permission = (
        "重要: 授予权限\n"
...
def resolve_sysom_auth(*, verify_api: bool = True) -> Dict[str, Any]:
    ...
    if env_result["available"]:
        got = _try_accept("环境变量 AKSK", env_result["credentials"])
        if got:
            return got
