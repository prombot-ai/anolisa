...
def summarize_precheck_for_agent(check_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    merged_ak = "AccessKey（环境变量与 ~/.aliyun 中 AK 模式，任选其一即可）"
    ...
    ram_rows = _find_rows_prefix(checked, "配置文件 EcsRamRole") + _find_rows_prefix(
        checked, "配置文件 RAM Role"
    )
    if ram_rows:
        ram_status = " | ".join(
    ...
    merged_rows = [
        ...
        {"label": "ECS RAM Role（配置文件 EcsRamRole / RAM Role）", "status": ram_status},
    ]
    ...
    if activation_needed:
        ...
        if file_ak_attempted and env_unconfigured and not env_attempted:
            ...
            focus = (
                "当前 ECS RAM Role 路径完成度更高：请优先完成 RAM 策略、实例绑定与 aliyun configure EcsRamRole，"
                "再处理 SysOM 控制台开通。"
            )
        else:
            primary_path = "configure_identity"
            focus = (
                "请先建立一种可用身份：环境变量 AK、~/.aliyun 中 AK，或（在 ECS 上）RAM 角色 + EcsRamRole；"
                "三条路线满足其一即可，不必并行配置多种。"
            )
    else:
        ...
        if ram_score > ak_score and (ecs_role_name or ram_rows):
            ...
            if ram_rows and ecs_role_name:
                focus = (
                    "实例已绑定 RAM 角色且 ~/.aliyun 为 EcsRamRole/RAM Role：请优先确认该角色具备 AliyunSysomFullAccess、"
                    "等待策略生效后重试 precheck；勿再并行配置 AccessKey。"
                )
            elif ecs_role_name:
                focus = (
                    f"实例已绑定 RAM 角色「{ecs_role_name}」：请授予 AliyunSysomFullAccess，"
                    "并在实例内将 aliyun CLI 配置为 EcsRamRole（或核对现有 EcsRamRole 配置）后重试 precheck；勿并行配置 AccessKey。"
                )
            else:
                focus = (
            ...
            if ecs_role_name or ram_rows:
                ...
                primary_path = "configure_identity"
                focus = (
                    "尚未形成明确领先路径：请先建立一种可用身份——环境变量 AK、~/.aliyun 中 AK，"
                    "或（在 ECS 上）实例 RAM 角色 + EcsRamRole；满足其一即可。"
                )
        else:
            primary_path = "configure_identity"
            focus = (
