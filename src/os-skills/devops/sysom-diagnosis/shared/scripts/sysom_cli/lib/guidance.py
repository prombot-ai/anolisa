...
DOC_PATHS_REL_SYSOM_DIAGNOSIS = {
    "openapi_permission_guide": "shared/references/openapi-permission-guide.md",
    "authentication": "shared/references/authentication.md",
...
INTERACTIVE_CONFIGURE_PTY_HINT = (
    ...
    "或使用 **/bash** 进入交互式 Bash，再在 sysom-diagnosis（技能根）执行 ./shared/scripts/osops configure。"
)


def precheck_commands() -> List[str]:
    """推荐在 sysom-diagnosis（技能根）执行；次选为在 shared/scripts 目录下已配置 PYTHONPATH 时直接调模块。"""
    return [
        "./shared/scripts/osops precheck",
        "cd shared/scripts && ./osops precheck",
    ]
...
def base_guidance_block() -> Dict[str, Any]:
    ...
    return {
        ...
        "credential_policy": (
            ...
            "RAM 用户 AK 路径须通过终端执行 ./shared/scripts/osops configure；"
            "密钥仅出现在用户本机终端输入，不进入聊天记录。"
            + INTERACTIVE_CONFIGURE_PTY_HINT
        ),
        "precheck_command": precheck_commands()[0],
        "configure_command": "./shared/scripts/osops configure",
        ...
        "guided_steps": [
            "1) 确认工作区含完整 sysom-diagnosis（含 shared/references/、shared/scripts/）；在技能根执行 ./shared/scripts/init.sh（仅首次或依赖变更后）。",
            "2) 可选：执行 ./shared/scripts/osops precheck 单独验证环境；远程子命令也会在调用 OpenAPI 前自动做同等检查。",
            "3) 若远程返回或 precheck 显示认证失败：请用户二选一——"
            "A) RAM 用户 AK（本机/非 ECS）B) ECS RAM Role（仅在阿里云 ECS 内）。"
            "选 A 则 Agent 用 Bash 执行 ./shared/scripts/osops configure（勿在聊天传密钥），再重试远程命令；"
            "无 PTY 时见 credential_policy 中的交互式 Shell 说明。"
            "选 B 则按 openapi-permission-guide §4.1 控制台绑角色 + aliyun configure --mode EcsRamRole。",
            "4) 若 service_not_activated：在 Alinux 控制台开通 SysOM（SLR 通常随开通自动创建）；"
            "子账号开通过程报权限不足时见仓库根 RAM 专文。",
            "5) 每完成一类配置后可再执行 precheck 或直接重试远程专项命令。",
            "6) 内存域：表述已对应专项则用本机 `./shared/scripts/osops memory memgraph`（占用高/大图）"
            "或 `memory oom`（OOM）或 `memory javamem`（Java）；"
            "仍不明时用 `memory classify`（均默认不调云）。选路见 references/memory-routing.md。"
            "深度/远程入口：`./shared/scripts/osops memory <classify|memgraph|oom|javamem> … --deep-diagnosis`；"
    ...
    return {
        ...
        "path_a_ram_user_ak": {
            ...
            "agent_run": (
                ...
                "./shared/scripts/osops configure。"
                + INTERACTIVE_CONFIGURE_PTY_HINT
            ),
        ...
        "path_b_ecs_ram_role": {
            ...
            "summary": (
                ...
                "ECS 控制台将角色绑定到当前实例；"
                "在实例内执行: aliyun configure --mode EcsRamRole --ram-role-name <角色名>；"
...
def remediation_auth_failed() -> List[str]:
    return [
        ...
        "若选 A：Agent 应在 sysom-diagnosis（技能根）直接执行 Bash — ./shared/scripts/osops configure（交互式本地输入，不回显）；"
        + INTERACTIVE_CONFIGURE_PTY_HINT
        + " 完成后执行 " + precheck_commands()[0] + "。",
        "若选 B：按 shared/references/openapi-permission-guide.md §4.1 与 shared/references/authentication.md「ECS RAM Role」— "
        f"在 RAM 控制台（{RAM_CONSOLE_URL}）与 ECS 控制台完成角色与绑定 + aliyun configure --mode EcsRamRole，再 "
        + precheck_commands()[0]
        + "。",
        "若 ~/.aliyun/config.json 解析失败：修复或删除后重试；选 A 可再次 ./shared/scripts/osops configure。",
    ]
...
def _remediation_ecs_ram_role_lines(
    *,
    ...
    if ram_profile_configured:
        lines.append(
            "步骤 3：已配置 ~/.aliyun 的 EcsRamRole/RAM Role，请核对 ram-role-name 与实例绑定一致、config.json 有效。"
        )
    elif role_label:
        lines.append(
            "步骤 3：实例内执行 aliyun configure --mode EcsRamRole，ram-role-name 与实例绑定一致。"
        )
    else:
        lines.append(
            "步骤 3：完成实例 RAM 角色绑定后，在实例内 aliyun configure --mode EcsRamRole。"
        )
...
def remediation_for_precheck_failure(
    ...
    *,
    ...
    ram_rows = [
        r
        for r in checked
        if str(r.get("method", "")).startswith(("配置文件 EcsRamRole", "配置文件 RAM Role"))
    ]
    if not prog:
        prog = {
    ...
    if primary == "access_key":
        ...
        head = [
            "主路径：RAM 用户 AccessKey（环境变量或 ~/.aliyun AK）。无需并行配置 ECS RAM Role。",
        ]
        if env_ok and not file_ok:
            return head + [
                f"步骤 1：在 RAM 控制台（{RAM_CONSOLE_URL}）确认 RAM 用户或 AK 对应主体已附加 AliyunSysomFullAccess。",
                "步骤 2：环境变量 AK 须与 precheck 在同一 shell 进程内导出后再执行；否则改用 ./shared/scripts/osops configure 写入 ~/.aliyun。",
                "步骤 3：执行: " + precheck_commands()[0],
            ]
        if file_ok and not env_ok:
            return head + [
                "步骤 1：核对 ~/.aliyun 中 AK 模式配置有效（密钥正确、profile 未损坏）。",
                f"步骤 2：在 RAM 控制台（{RAM_CONSOLE_URL}）确认该 RAM 用户已附加 AliyunSysomFullAccess。",
                "步骤 3：执行: " + precheck_commands()[0],
            ]
        if file_bad and not env_ok:
            return head + [
                "步骤 1：在技能根由 Agent 执行 ./shared/scripts/osops configure 重建有效配置，勿在对话中传输 Secret。",
                f"步骤 2：在 RAM 控制台（{RAM_CONSOLE_URL}）确认 RAM 用户已附加 AliyunSysomFullAccess。",
                "步骤 3：执行: " + precheck_commands()[0],
            ]
        return head + [
            "步骤 1：在技能根由 Agent 执行 ./shared/scripts/osops configure，勿在对话中传输 Secret。",
            f"步骤 2：在 RAM 控制台（{RAM_CONSOLE_URL}）确认 RAM 用户已附加 AliyunSysomFullAccess。",
...
def diagnosis_subsystem_minimal_guidance(
    *,
    ...
    认证/开通类问题仍以 shared/references/openapi-permission-guide.md 为准。
    """
    rem: List[str] = [
