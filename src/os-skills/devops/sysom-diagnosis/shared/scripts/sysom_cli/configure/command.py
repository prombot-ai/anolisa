...
def _envelope_non_interactive_shell() -> Dict[str, Any]:
    ...
    remediation = [
        ...
        "方式 1（优先）：在 **COSH** 中执行 **/settings**，使能「**交互式Shell（PTY）**」，再在 sysom-diagnosis（技能根）重试: ./shared/scripts/osops configure",
        "方式 2：执行 **/bash** 进入交互式 Bash，cd 到技能根后执行: ./shared/scripts/osops configure",
        "方式 3：按 shared/references/authentication.md「方式 A: 手动编辑配置文件」编辑 ~/.aliyun/config.json（mode=AK），勿在聊天粘贴 Secret",
        f"配置完成后执行: {pc0}",
    ]
    detail_lines = "\n".join(
        [
            "1) /settings → 开启「交互式Shell（PTY）」→ 技能根重试 ./shared/scripts/osops configure",
            "2) /bash → cd <sysom-diagnosis 技能根> → ./shared/scripts/osops configure",
    ...
    return envelope(
        ...
        data={
            ...
            "read_next": [
                "shared/references/authentication.md",
            ],
            "guidance": {
...
class ConfigureCommand(BaseCommand):
    ...
    def execute_local(self, ns: Namespace) -> Dict[str, Any]:
        ...
        return envelope(
            ...
            agent=agent_block(
                status="success",
                summary=f"已写入 {path}（profile={profile_name}, mode=AK）。请执行 ./shared/scripts/osops precheck 验证。",
                findings=[
                    {
                        "severity": "info",
                        "title": "下一步",
                        "detail": "在 sysom-diagnosis（技能根）执行: ./shared/scripts/osops precheck",
                    }
                ],
            ),
            data={
