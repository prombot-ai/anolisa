...
2. **检查环境变量**：验证 `ALIBABA_CLOUD_ACCESS_KEY_ID` 和 `ALIBABA_CLOUD_ACCESS_KEY_SECRET`
3. **检查配置文件**：解析 `~/.aliyun/config.json` 中的认证配置（AK 或 EcsRamRole 模式）
...
**注意**：请使用 `osops precheck` 验证配置，不要使用 `aliyun ecs DescribeInstances` 等命令，因为 SysOM 需要特定的 API 权限。
...
请优先在**本机终端**（由 Agent 调用 Bash 或用户自己打开终端），在 sysom-diagnosis（技能根）执行 **`./shared/scripts/osops configure`**（交互式写入 `~/.aliyun/config.json`），再执行 **`./shared/scripts/osops precheck`**。若必须用环境变量，仅在**同一 shell 一条命令**内：`export ... && ./shared/scripts/osops precheck`，且**勿在聊天中发送密钥**。

若 Agent 调用的终端**不支持交互**（无 PTY），`osops configure` 无法读入密钥：在 **COSH** 中可通过 **`/settings`** 使能「**交互式Shell（PTY）**」，或使用 **`/bash`** 进入交互式 Bash，再在技能根执行 **`./shared/scripts/osops configure`**。
...
3. 在 sysom-diagnosis（技能根）重新执行：`./shared/scripts/osops configure`
...
2. 等待 2-5 分钟让权限生效
3. 重新运行 `./shared/scripts/osops precheck`
