...
version: 1.13.0
description: 面向 Linux 内存问题做深度分析与诊断。覆盖内存高/不足、系统或应用 OOM、oom-killer、Java 应用内存与内存全景大图。输出结构化诊断信封供自动化续跑。
...
# SysOM 内存诊断（sysom-diagnosis）

在 **`sysom-diagnosis/`**（技能根）下使用 **`./shared/scripts/osops`** 做内存诊断。stdout 为 **JSON 信封**，默认**本机快速排查不调云**；远程专项由 **`--deep-diagnosis`** 触发，内建环境检查。

## Agent 核心行为

1. **本机优先**：先跑 memory quick，不要默认索要 region/instance。
2. **意图路由**：占用高/大图 → `memgraph`；OOM/oom-killer → `oom`；Java → `javamem`；不明 → `classify`。详见 [memory-routing.md](./references/memory-routing.md)。
...
./shared/scripts/osops memory memgraph --deep-diagnosis --channel ecs --timeout 300  # 远程专项
```
...
| 需求 | 文档 |
|------|------|
| 意图→子命令映射 | [memory-routing.md](./references/memory-routing.md) |
