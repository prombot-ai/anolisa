...
export async function parseArguments(): Promise<CliArgs> {
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('co')
    .usage(
      'Usage: co [options] [command]\n\nCopilot Shell - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    .option('telemetry', {
      type: 'boolean',
      description:
    ...
    .option('proxy', {
      type: 'string',
      description:
        'Proxy for Copilot Shell, like schema://user:password@host:port',
    })
    .deprecateOption(
      'proxy',
...
export async function loadCliConfig(
  ...
  if (argv.continue || argv.resume) {
    ...
    if (argv.resume) {
      ...
      if (!sessionData) {
        const message = `No saved session found with ID ${argv.resume}. Run \`co --resume\` without an ID to choose from existing sessions.`;
        console.log(message);
        process.exit(1);
      }
    }
  ...
  const config = new Config({
    ...
    enableHooks: settings.hooksConfig?.enabled !== false,
