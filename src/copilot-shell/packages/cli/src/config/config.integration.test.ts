...
import { Config } from '@copilot-shell/core';
...
vi.mock('@copilot-shell/core', async () => {
  const actual = await vi.importActual('@copilot-shell/core');
  return {
    ...actual,
...
describe('Configuration Integration Tests', () => {
  ...
  describe('Skill-OS Configuration', () => {
    ...
    it('should return undefined and be disabled when skillOS is not configured', () => {
      const configParams: ConfigParameters = {
        cwd: '/tmp',
        generationConfig: TEST_CONTENT_GENERATOR_CONFIG,
        embeddingModel: 'test-embedding-model',
    ...
    it('should use configured Skill-OS baseUrl and be enabled when skillOS.baseUrl is provided', () => {
      const configParams: ConfigParameters = {
        cwd: '/tmp',
        generationConfig: TEST_CONTENT_GENERATOR_CONFIG,
        embeddingModel: 'test-embedding-model',
        ...
        debugMode: false,
        skillOS: {
          baseUrl: 'https://my-internal-skill-server.example.com',
        },
      };

      const config = new Config(configParams);

      expect(config.getSkillOSUrl()).toBe(
        'https://my-internal-skill-server.example.com',
      );
