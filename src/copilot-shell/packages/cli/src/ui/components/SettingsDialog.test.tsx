...
describe('SettingsDialog', () => {
  ...
  describe('Settings Navigation', () => {
    it('should navigate down with arrow key', async () => {
      ...
      // Wait for the state update to be reflected in the UI before asserting
      await waitFor(() => {
    ...
    it('wraps around when at the top of the list', async () => {
      ...
      // Wait for wrap-around to be reflected in the UI
      await waitFor(() => {
  ...
  describe('Specific Settings Behavior', () => {
    ...
    it('should show restart prompt for restart-required settings', async () => {
      ...
      expect(lastFrame()).not.toContain(
        'To see changes, Copilot Shell must be restarted',
      );
    ...
    it('should keep restart prompt when switching scopes', async () => {
      ...
      stdin.write(TerminalKeys.DOWN_ARROW as string);
      // Wait for navigation to Language: UI to complete before pressing Enter.
      // Then wait an extra tick to ensure the keypress handler has re-registered
      // with the updated activeSettingIndex (avoids stale closure reading index=0).
      await waitFor(() => {
  ...
  describe('Output Language', () => {
    it('treats empty output language as auto', async () => {
      ...
      // Wait for navigation to Language: Model to complete before pressing Enter
      const outputLanguageLabel =
        getSettingDefinition('general.outputLanguage')?.label ??
        'Language: Model';
      await waitFor(() => {
        expect(lastFrame()).toContain(`● ${outputLanguageLabel}`);
      });
      await wait(); // extra tick: ensure keypress handler re-registered with new activeSettingIndex
  ...
  describe('Complex User Interactions', () => {
    ...
    it('should allow changing multiple settings without losing pending changes', async () => {
      ...
      // Wait for initial render and confirm focus on first setting
      await waitFor(() => {
        expect(lastFrame()).toContain('● Tool Approval Mode');
      });

      // Toggle first setting (Tool Approval Mode, requires restart)
      stdin.write(TerminalKeys.ENTER as string);
      await wait();

      // Navigate to next setting and wait for UI to confirm navigation
      stdin.write(TerminalKeys.DOWN_ARROW as string);
      await waitFor(() => {
        expect(lastFrame()).toContain('● Language: UI');
      });
      ...
      // Navigate to the next setting and wait for UI to confirm navigation
      stdin.write(TerminalKeys.DOWN_ARROW as string);
      await waitFor(() => {
        expect(lastFrame()).toContain('● Language: Model');
      });
      await wait(); // extra tick: ensure keypress handler re-registered with new activeSettingIndex
      stdin.write(TerminalKeys.ENTER as string);
      await wait();
