  ...
  describe('Command Parsing and Matching', () => {
    it('should be case-sensitive', async () => {
      ...
      await act(async () => {
        ...
        returnValue = await result.current.handleSlashCommand('/Test');
      });

      // '/Test' does not match the registered 'test' command (case-sensitive),
      // so it should not be treated as a command at all.
