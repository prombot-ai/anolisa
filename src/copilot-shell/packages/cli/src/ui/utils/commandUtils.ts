 ...
 *
 * When an optional `commands` list is provided the function additionally
 * verifies that the first word after '/' matches a registered command name
 * or alias.  This keeps the "is known command" logic in a single place so
 * that callers (highlighting, message rendering, routing) don't duplicate it.
 *
 * @param query The input query string.
 * @param commands Optional list of registered slash commands.
 * @param prefixMatch When true, the first word only needs to be a prefix of
 *   a known command name (useful for real-time input highlighting while the
 *   user is still typing).  Defaults to false (exact match).
 * @returns True if the query looks like an '/' command, false otherwise.
 */
export const isSlashCommand = (
  ...
  // When a commands list is supplied, only return true if the first word
  // matches a known command name or alias.
  if (commands && commands.length > 0) {
    const firstWord = query.trim().substring(1).trim().split(/\s+/)[0] || '';
    if (prefixMatch) {
      return commands.some(
        (cmd) =>
          cmd.name.startsWith(firstWord) ||
          (cmd.altNames?.some((alt) => alt.startsWith(firstWord)) ?? false),
      );
    }
    return commands.some(
      (cmd) =>
        cmd.name === firstWord || (cmd.altNames?.includes(firstWord) ?? false),
    );
  }
