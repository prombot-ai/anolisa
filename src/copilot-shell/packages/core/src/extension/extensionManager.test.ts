...
function createExtension({
  ...
  if (addContextFile) {
    fs.writeFileSync(path.join(extDir, 'COPILOT.md'), 'context');
  }
...
describe('extension tests', () => {
  ...
  describe('loadExtension', () => {
    ...
    it('should load context file path when COPILOT.md is present', async () => {
      createExtension({
        extensionsDir: userExtensionsDir,
        name: 'ext1',
        version: '1.0.0',
      ...
      expect(ext1?.contextFiles).toEqual([
        path.join(userExtensionsDir, 'ext1', 'COPILOT.md'),
      ]);
    ...
    it('should use default COPILOT.md when contextFileName is empty array', async () => {
      ...
      fs.writeFileSync(path.join(extDir, 'COPILOT.md'), 'context content');
      ...
      expect(ext?.contextFiles).toEqual([
        path.join(userExtensionsDir, 'ext-empty-context', 'COPILOT.md'),
      ]);
    });
