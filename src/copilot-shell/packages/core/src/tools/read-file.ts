...
> {
  ...
  protected override validateToolParamValues(
    ...
    const remoteSkillsDir = this.config.storage.getRemoteSkillsDir();
    // Use an interface check in case of older version without the method
    const globalRemoteSkillsDir =
    ...
    const isWithinRemoteSkills =
      isSubpath(remoteSkillsDir, resolvedFilePath) ||
      (globalRemoteSkillsDir !== '' &&
        isSubpath(globalRemoteSkillsDir, resolvedFilePath));
    const isWithinSystemSkills = isSubpath(
    ...
    if (fileService.shouldQwenIgnoreFile(params.file_path)) {
      return `File path '${filePath}' is ignored by .copilotignore pattern(s).`;
    }
