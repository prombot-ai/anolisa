...
const SUDO_FOREGROUND_TIMEOUT_MS = 300000; // 5 minutes for sudo commands

/** Check if a command invokes sudo or su. */
const hasSudoCommand = (command: string): boolean =>
  /\bsudo\b|\bsu\b/.test(command);
...
> {
  ...
  async execute(
    ...
    const isSudoCommand = hasSudoCommand(strippedCommand);

    const effectiveTimeout = this.params.is_background
      ? undefined
      : (this.params.timeout ??
        (isSudoCommand
          ? SUDO_FOREGROUND_TIMEOUT_MS
          : DEFAULT_FOREGROUND_TIMEOUT_MS));

    // Force PTY mode for sudo commands so they get a proper terminal
    const shouldUsePty =
      this.config.getShouldUseNodePtyShell() || isSudoCommand;
          ...
          (event: ShellOutputEvent) => {
            ...
            switch (event.type) {
              ...
              case 'password_prompt':
                // Password prompt detected; notify the UI to focus the shell input.
                if (onPasswordPrompt) {
                  onPasswordPrompt();
                }
                break;
      ...
      Kind.Execute,
      {
        ...
        properties: {
          ...
          is_background: {
            type: 'boolean',
            description:
              'Whether to run the command in background. Set to true for long-running processes like development servers, watchers, or daemons that should continue running without blocking further commands.',
          },
          timeout: {
            type: 'number',
