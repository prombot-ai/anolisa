 ...
 * This combines the shared patterns with dynamic patterns like COPILOT.md.
...
${finalExclusionPatternsForDescription
  ...
  .join(
    ...
    // Add a note if .copilotignore patterns contributed to the final list of exclusions
    if (qwenIgnorePatterns.length > 0) {
      const geminiPatternsInEffect = qwenIgnorePatterns.filter((p) =>
        finalExclusionPatternsForDescription.includes(p),
      ).length;
      if (geminiPatternsInEffect > 0) {
        excludeDesc += ` (includes ${geminiPatternsInEffect} from .copilotignore)`;
      }
    }
  ...
  constructor(private config: Config) {
    const parameterSchema = {
      ...
      properties: {
        ...
        file_filtering_options: {
          description:
            'Whether to respect ignore patterns from .gitignore or .copilotignore',
          type: 'object',
          properties: {
            respect_git_ignore: {
              description:
