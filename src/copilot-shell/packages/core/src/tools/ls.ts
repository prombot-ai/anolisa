...
export interface LSToolParams {
  ...
  /**
   * Whether to respect .gitignore and .copilotignore patterns (optional, defaults to true)
   */
...
export class LSTool extends BaseDeclarativeTool<LSToolParams, ToolResult> {
    ...
    super(
      ...
      Kind.Search,
      {
        properties: {
          ...
          file_filtering_options: {
            description:
              'Optional: Whether to respect ignore patterns from .gitignore or .copilotignore',
            type: 'object',
            properties: {
              respect_git_ignore: {
                description:
              ...
              respect_qwen_ignore: {
                description:
                  'Optional: Whether to respect .copilotignore patterns when listing files. Defaults to true.',
                type: 'boolean',
              },
            },
          },
  ...
  protected override validateToolParamValues(
    ...
    const remoteSkillsBase = this.config.storage.getRemoteSkillsDir();
    const globalRemoteSkillsBase =
      typeof this.config.storage.getGlobalRemoteSkillsDir === 'function'
        ? this.config.storage.getGlobalRemoteSkillsDir()
        : '';
    const isUnderUserSkills = isSubpath(userSkillsBase, params.path);
    const isUnderRemoteSkills =
      isSubpath(remoteSkillsBase, params.path) ||
      (globalRemoteSkillsBase !== '' &&
        isSubpath(globalRemoteSkillsBase, params.path));
    const isUnderSystemSkills = isSubpath(
