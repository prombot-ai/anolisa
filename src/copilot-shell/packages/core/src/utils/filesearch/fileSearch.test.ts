...
describe('FileSearch', () => {
  ...
  it('should use .copilotignore rules', async () => {
    ...
    expect(results).toEqual(['src/', '.copilotignore', 'src/not-ignored.js']);
  });

  it('should combine .gitignore and .copilotignore rules', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': 'dist/',
      '.copilotignore': 'build/',
