...
async function getGeminiMdFilePathsInternalForEachDir(
  ...
  for (const geminiMdFilename of geminiMdFilenames) {
    ...
    if (isHomeDirectory) {
      // For home directory, only check for COPILOT.md directly in the home directory
...
/**
 * Loads hierarchical COPILOT.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  if (filePaths.length === 0) {
    if (debugMode) logger.debug('No COPILOT.md files found in hierarchy.');
  ...
  // Only count files that match configured memory filenames (e.g., COPILOT.md),
