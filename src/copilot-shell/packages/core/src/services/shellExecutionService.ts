...
/** Pattern to detect sudo/su password prompts in terminal output. */
const SUDO_PASSWORD_PROMPT_PATTERN =
  /(?:\[sudo\]\s*password|Password:|password for \S+:|密码：)/i;
...
export type ShellOutputEvent =
  ...
  | {
      /** Signals that a password prompt (e.g. sudo) has been detected. */
...
export class ShellExecutionService {
  ...
  static async execute(
    ...
    if (shouldUseNodePty) {
      ...
      if (ptyInfo) {
        try {
          console.log(
            `[ShellExec] Using PTY (${ptyInfo.name}) for: ${commandToExecute.substring(0, 80)}`,
          );
          ...
          console.warn(
            '[ShellExec] PTY spawn failed, falling back to child_process:',
            _e,
          );
        }
      } else {
        console.warn(
          '[ShellExec] PTY requested but not available (getPty returned null), falling back to child_process',
        );
      }
    }
  ...
  private static childProcessFallback(
    ...
    try {
      ...
      // When a sudo/su command was meant to run in PTY mode but node-pty
      // is unavailable, inherit the parent process's stdin so that the
      // real TTY is passed through for password input.
      const needsStdin =
        requestedPty && /\bsudo\b|\bsu\b/.test(commandToExecute);
      const stdinMode = needsStdin ? 'inherit' : 'ignore';

      if (needsStdin) {
        console.log(
          '[ShellExec] child_process fallback: using stdin=inherit + detached=false for sudo command',
        );
      }
      ...
      // For sudo commands: detached must be false because detached=true calls
      // setsid() which creates a new session without a controlling terminal,
      // causing sudo to fail with "a terminal is required to read the password".
  ...
  private static executeWithPty(
    ...
    try {
      ...
      const result = new Promise<ShellExecutionResult>((resolve) => {
        ...
        let passwordPromptEmitted = false;
            ...
            () =>
              new Promise<void>((resolve) => {
                ...
                if (isStreamingRawContent) {
                  ...
                  // Detect sudo/su password prompts and notify the UI
                  if (
                    !passwordPromptEmitted &&
                    SUDO_PASSWORD_PROMPT_PATTERN.test(decodedChunk)
                  ) {
                    passwordPromptEmitted = true;
                    onOutputEvent({ type: 'password_prompt' });
                  }
