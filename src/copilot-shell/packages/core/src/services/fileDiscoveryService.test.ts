...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .copilotignore patterns even when not in a git repo', async () => {
      await createTestFile('.copilotignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.copilotignore', 'logs/');
    });
  ...
  describe('shouldGitIgnoreFile & shouldQwenIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.copilotignore', '*.log');
    });
